...
pt = 3 %variables
k = 0.05
func = @(x) (((x)./(1-x)).*(sqrt((2.*pt)./(2+x))))-k %function we want to approximate the root location of
...
%being that this function has a point where it diverges to infinity, it can
%be assumed that a closed bracket method would be more accurate

function [root, fx, ea, iter] = falsePosition (func, xl ,xu, es, maxiter) %bracket false position method
